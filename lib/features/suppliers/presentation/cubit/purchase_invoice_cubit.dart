...
class PurchaseInvoiceCubit extends Cubit<PurchaseInvoiceState> {
  ...
  void updateInvoiceDate(DateTime date) =>
      emit(state.copyWith(invoiceDate: date, error: null));
  ...
  Future<void> saveInvoice(String supplierId, String? invoiceNumber) async {
    ...
    final res = await supplierRepository.createPurchaseInvoice(
      supplierId: supplierId,
      pharmacyId: pharmacyId,
      invoiceNumber: invoiceNumber,
      date: state.invoiceDate ?? DateTime.now(), // G4: Use custom date
  ...
  double _calcTotal(List<Map<String, dynamic>> items) =>
      items.fold(0.0, (sum, i) => sum + ((i['quantity'] as num) * (i['purchasePrice'] as num)));
}
