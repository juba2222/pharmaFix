...
class SupplierReadService {
  ...
  Stream<List<SupplierEntity>> watchAll(SupplierSortOption sort) {
    ...
    return query.watch().map((rows) => rows.map((r) => SupplierMappers.fromTable(r)).toList());
  }

  Future<Map<String, dynamic>> getStats(String supplierId) async {
    final invoices = await (db.select(db.purchaseInvoicesTable)..where((t) => t.supplierId.equals(supplierId))).get();
  ...
  Future<List<SupplierStatementItem>> getStatement(String supplierId) async {
    final rows = await (db.select(db.purchaseInvoicesTable)..where((t) => t.supplierId.equals(supplierId))).get();
  ...
  Future<List<Map<String, dynamic>>> getCatalog(String supplierId) async {
    final query = db.select(db.purchaseInvoiceItemsTable).join([
      innerJoin(db.productsTable, db.productsTable.id.equalsExp(db.purchaseInvoiceItemsTable.productId)),
      innerJoin(db.purchaseInvoicesTable, db.purchaseInvoicesTable.id.equalsExp(db.purchaseInvoiceItemsTable.purchaseInvoiceId)),
    ])..where(db.purchaseInvoicesTable.supplierId.equals(supplierId));

    final rows = await query.get();
    return rows.map((r) => {'id': r.readTable(db.productsTable).id, 'name': r.readTable(db.productsTable).localName, 'barcode': r.readTable(db.productsTable).barcode}).toList();
  }
}
